...
# The last tag, 3.10, still uses the AC_CANONICAL_SYSTEM macro, so the
# maintained branch is pinned instead, where it has been replaced.
...
# configure.ac takes the package version from build/current-version, a file
# upstream generates in autogen.sh out of git metadata. autogen.sh is not used
# here, as the autotools class runs autoreconf itself, so write the file
# directly. Without it the package version ends up empty.
